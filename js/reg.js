...
//     myform.onsubmit = proverka /*привязываем событие onsubmit по наступлению которого 
//                                 будет запускаться функция proverka*/
// }
...
//             if (!document.getElementById("metka")){ /*условие по которому функция будет 
//                                                       определять открыта ли форма или нет*/
//                     for (var i=1; i<=6; i++){  /*если форма закрыта, то циклом по таймауту
//                                                    запускаем функции добавления полей*/
//                             var p = "pole"+i+"()";//формирование названия нашей функции 
//                             var t1 = i * 100;//расчет времени таймаута
//                             var t =100 + t1;//расчет времени таймаута
//                             setTimeout (p,t)/*запуск функций через определенное
//                                               количество времени*/
//                     }
//     var myIn = document.getElementById("myIn");/*находим кнопку «расширенная форма» 
//                                                                  по id*/
//     myIn.setAttribute("value", "  Обычная форма  " );/*меняем название 
//                                                      кнопки на «обычная форма»*/
//     myIn.removeAttribute("ONCLICK");/*удаляем атрибут ONCLICK, для того 
//                                      чтобы в процессе выполнения кода сам 
//                                        код нельзя было запустить еще раз*/  
//     }
//     else
//             isNormal();/*если форма уже открыта, то запускаем функцию по удалению 
//                                 полей для расширенной формы*/ 
// }
// //функция по запуску плавного удаления полей формы
// function isNormal(){
//     var     form = document.getElementById("myTable");//находим элемент таблица по ее id
//     var del = form.getElementsByTagName("TR");//находим все теги tr в нашей таблице
//             for (var i = 9; i < del.length; i--){ /*цикл по запуску функций для 
//                                                           удаления полей, по таймауту*/
//                     if (i==3){ /*если цикл доходит до 3 поля то он прекращается, 
//                                             чтобы у нас обязательные поля не удалились*/ 
//                             break;
//                     } else 
//                     var fun = "deleteTR"+"("+i+")";/*формирование названия функции по 
//                                                         удалению нашей строки в таблице, 
...
//             myIn.setAttribute("value", "Расширенная форма" );/*меняем название кнопки на 
//                                                                   «расширенная форма»,      
...
//             //условие проверки полей
//             if (proverkaName.value=="" || proverkaEmail.value=="" || proverkaPsw.value==""){
//                     proverkaName = (proverkaName.value) ? "" : "Поле - имя\n"
// /*если в поле «имя» пусто то в переменную  proverkaName вставляем "Поле – имя", для 
// вывода в предупредительном сообщении*/
//                     proverkaEmail = (proverkaEmail.value) ? "" : "Поле - email\n"
// /*если в поле «email» пусто то в переменную proverkaEmail вставляем "Поле – email", для 
// вывода в предупредительном сообщении */
//                     proverkaPsw = (proverkaPsw.value) ? "" : "Поле - пароль\n"
// /*если в поле «пароль» пусто то в переменную proverkaPsw вставляем 
// "Поле – пароль", для вывода в предупредительном сообщении */
// alert ("Не заполнены следующие обязательные поля:\n"+proverkaName+proverkaEmail+proverkaPsw);
// /*формируем предупредительное сообщение */
//     return  false;/*прекращаем выполнения события onsubmit, 
// для того чтобы наши «неправильные» данные не отправились на сервер */
//             }
//     else{
//     //сообщение что данные заполнены правильно и отправились на сервер
// alert ("Данные успешно отправлены на сервер\nПримечание:\nНа самом деле эти данные \
// никуда не отправлены, Вам сначала необходимо настроить прием этих данных на своем \
// сервере, т.е. настроить серверные скрипты");
...
//     for (var i = 0; i < alCentr.length; i++){/*циклом переберем все строки 
//                                                  таблицы добавляем им атрибут align*/ 
...
//     myIn.setAttribute("ONCLICK","isMore()");//возвращаем ей атрибут ONCLICK
// }
